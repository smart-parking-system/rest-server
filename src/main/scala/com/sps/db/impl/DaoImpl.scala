...
import com.sps.db.{Connection, ConnectionInfo, Model, Table, Dao}
 ...
 */
class DaoImpl[T <: Model](connInfo: ConnectionInfo, tableName: String, primaryKeyName: String, creator: () => T)(implicit ct: ClassTag[T]) extends Dao[T] {
  ...
  def getBy(fieldName: String, value: String): List[T] = {
    ...
    try {
      val stmt = connection.createStatement()
      val rs = stmt.executeQuery(s"""SELECT * FROM $tableName WHERE $fieldName='$value' """)
      val fields = Reflection.getFields[T]
      while (rs.next) {
        val value = creator()
        fields.foreach(f => value.setField(f.getName(), rs.getObject(f.getName())))
